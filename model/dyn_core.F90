...
!!! Currently divgd is always 0.0 in the regional domain boundary area.
    ...
    !$ser data delpcd=vt delpd=delp ptd=pt ud=u vd=v wd=w ucd=uc vcd=vc uad=ua vad=va divgdd=divgd mfxd=mfx mfyd=mfy cxd=cx cyd=cy crxd=crx cryd=cry xfxd=xfx yfxd=yfx q_cond=q_con zhd=zh heat_sourced=heat_source diss_estd=diss_est zvir=zvir nq=nq dt=dt nord_v=nord_v damp_vt=damp_vt
    !$ser off
    !$ser verbatim end if
...
! Average horizontal "convergence" to cell center
    ...
    !$ser data delpcd=vt delpd=delp ptd=pt ud=u vd=v wd=w ucd=uc vcd=vc uad=ua vad=va divgdd=divgd mfxd=mfx mfyd=mfy cxd=cx cyd=cy crxd=crx cryd=cry xfxd=xfx yfxd=yfx q_cond=q_con heat_sourced=heat_source diss_estd=diss_est nord_vd=nord_v damp_vtd=damp_vt
    !$ser off
    !$ser verbatim end if
    if (flagstruct%regional) then
       call mpp_update_domains(uc, vc, domain, gridtype=CGRID_NE)
